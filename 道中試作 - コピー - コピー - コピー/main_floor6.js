                ...
                if(patern==10){
                   ...
                   rads2 = 1;
                    ...
                    enemy2.on(Event.ENTER_FRAME,function(){
                            ...
                            if (middlebullet.length <= 10 || enemy2.centerY>300||delay>deal||komes.length<=10) return;
...
// Random = random(30,40);
                            for (i = a; i <= b; i++) {
                                // for(r = -4; r <= 4; r ++){
                            if (tamakawa.length <= 10 || enemy2.centerY>300||komes.length<=10||middlebullet.length<=10) return;
                            if(delay<=deal){
                        //    tamakawa= objectpool2.pop();
            ...
            // kome = komes.pop();
            // kome.centerX = this.centerX+ (Math.abs(angle2/3)* Math.cos(((angle2 + 45*r)) * (Math.PI / 180)));
            // kome.centerY = this.centerY+ (Math.abs(angle2/3)* Math.sin(((angle2+ 45*r)) * (Math.PI / 180)));
            //                 // radian = 0;
            //                 // radian = Math.atan2(tama.centerY - playerEntityGroup.y, tama.centerX - playerEntityGroup.x);
            //                 radian = Math.atan2(kome.centerY - this.centerY, kome.centerX - this.centerX);
            //                 angle = radian / (Math.PI / 180) + 180;
            //                 angle += angle2/4
            //                 // 距離と角度から座標 
            //                 // angle = 90;
            //                 // angle = (360/c)*i+rad*(360/62)+rad4;
            ...
            //                 kome.moveY = 6 * Math.sin(angle * (Math.PI / 180));
            kome = komes.pop();
            // middletama.scale(0.7,0.7);
            kome.centerX = 360
            kome.centerY = (390/12)*i-90
                            // radian = 0;
                            // radian = Math.atan2(tama.centerY - playerEntityGroup.y, tama.centerX - playerEntityGroup.x);
                            // radian = Math.atan2(middletama.centerY - this.centerY, middletama.centerX - this.centerX);
                            // angle = radian / (Math.PI / 180) + 180;
                            angle =random(0,90)+90;
                            kome.rotation= -90+angle
                            // 距離と角度から座標 
                            // angle = 90;
                            // angle = (360/c)*i+rad*(360/62)+rad4;
                            // angle = (360/c)*i+rad*(360/62)+random(-10,10)+rad4;
                            //  angle = (360/c)*i+random(0,360);
                            // middletama.rotation = angle-90;
                            // angle = (360/c)*i+rad*1+rad2*1+90+rad3;
                            // tamakawa.moveX = 7 * Math.cos(angle * (Math.PI / 180));
                            // tamakawa.moveY = 7  * Math.sin(angle * (Math.PI / 180));
                            kome.moveX = random(20,30)/10 * Math.cos(angle * (Math.PI / 180));
                           kome.moveY = random(20,30)/10 * Math.sin(angle * (Math.PI / 180));
                        ...
                        //  }  
                           ...
                           if (isOutside(this)) { 
                                ...
                                for (i = a; i <= b; i++) {
                                    ...
                                    if(delay<=deal&& phase>0){
                                        middletama= middlebullet.pop();
                                        middletama.centerX = random(0,360);
                                        middletama.centerY = 0;
                                        angle=90;
                                        middletama.moveX = 4 * Math.cos(angle * (Math.PI / 180));
                                        middletama.moveY = 4 * Math.sin(angle * (Math.PI / 180));
                                //    tamakawab= objectpool3.pop();
                                //     tamakawab.centerX = this.centerX;
                                //     tamakawab.centerY = this.centerY;
                            //     aokome = aokomes.pop();
                            //     aokome.centerX =this.centerX-(125*Math.cos((-angle2) * (Math.PI / 180))) +Random*i;
                            //     aokome.centerY = this.centerY- (50* Math.sin(((-angle2))* (Math.PI / 180)) );
                            //     // aokome.centerY = this.centerY;
                            //         // radian = 0;
                            //         // radian = Math.atan2(tama.centerY - playerEntityGroup.y, tama.centerX - playerEntityGroup.x);
                            //         // angle = radian / (Math.PI / 180) + 180;
                            //         angle = 90;
                            ...
                            //         aokome.moveX = 4 * Math.cos(angle * (Math.PI / 180));
                            //         aokome.moveY = 4   * Math.sin(angle * (Math.PI / 180));

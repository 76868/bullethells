...
baria.scale(2.0,2.0)
    ...
    // bullet();   
        ...
        player.on(Event.ENTER_FRAME, function () {  
            // console.log(bmtime,inb,bombs,komes.length);
            if(bmtime>0){
                bmtime-=1;
                bframe +=1
   ...
   floor6Start();
      if(count>0){
        count-=1;
            ...
            if(objectpool5.length<=1||count>0||enemies.length ==0)return;
           ...
           if (this.age % 12 == 0 ){
            ...
            kome.centerY = this.centerY;
                           ...
                           if (isOutside(this)) { 
                                ...
                                for (i = a; i <= b; i++) {
                                    if (tamakawab.length <= 10 || enemy2.centerY>300||komes.length<=10) return;
                                    if(delay<=deal&& phase>21){
